...
/**
 * One compiled permission policy package. `policyData` is the exact
 * abi.encode(Permission[]) payload the pinned CallPolicy module receives after
 * the permission ID prefix; every allowed (target, selector) pair carries the
 * native value ceiling that applies to it.
 */
