 ...
 * are one moduleType 5 policy bounding the calls and their value, followed by one
 * moduleType 6 signer carrying the key's public material. Policies are required,
 * so an unscoped session cannot be expressed at all.
 *
...
export function sessionOperator(value: SessionOperatorInput): Readonly<OperatorProfile> {
  ...
  const permissionId = derivePermissionId(
    policy.module,
    policy.policyData,
    signer,
    key.kind,
    ...
    resolvePackages: (deployment: Readonly<KernelV4Deployment>) => {
      ...
      return Object.freeze([
        Object.freeze({
          ...
          moduleData: concat([paddedPermissionId, policy.policyData]),
