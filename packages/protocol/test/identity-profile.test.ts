...
function expectProfileError(action: () => unknown, code: OaathIdentityProfileError["code"]): void {
  try {
    action();
  } catch (error) {
    expect(error).toBeInstanceOf(OaathIdentityProfileError);
...
describe("identity profile codecs", () => {
  ...
  it("accepts only the first-party ECDSA and WebAuthn operator shapes", () => {
    expect(OAATH_OPERATOR_CREDENTIAL_PROFILE_VERSION).toBe("oaath.operator-credential-profile/v1");
  ...
  it("owns one exact Kernel profile and binds one caller-supplied action chain", () => {
    expect(OAATH_KERNEL_ACCOUNT_PROFILE_VERSION).toBe("oaath.kernel-account-profile/v1");
  ...
  it("rejects versions, contradictory fields, extras, symbols, and accessors", () => {
    for (const value of [
      { ...ownerEcdsa, version: "oaath.owner-credential-profile/v0" },
      { version: "oaath.owner-credential-profile/v1", kind: "ecdsa" },
    ...
    for (const replacement of [
      { ...accountProfile, version: "oaath.kernel-account-profile/v0" },
