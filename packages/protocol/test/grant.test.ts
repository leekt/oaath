...
import { createGrant, type GrantIdentity, OaathGrantError, parseGrant } from "../src/index.js";
...
function expectGrantError(action: () => unknown, code: OaathGrantError["code"]): void {
  try {
    action();
  } catch (error) {
    expect(error).toBeInstanceOf(OaathGrantError);
    expect((error as OaathGrantError).code).toBe(code);
    return;
  }
...
describe("Grant current codec", () => {
  ...
  it("rejects accessors without invocation and sanitizes reflection failures", () => {
    ...
    const hostile = new Proxy(
      ...
      {
        getPrototypeOf() {
          throw new OaathGrantError("grant_record_invalid", secret);
        },
      },
    );
    try {
