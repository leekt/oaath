...
export interface PermissionSessionSigner {
  ...
  /** Deployment-registered signer provider identity. */
...
export interface PermissionRequest {
  ...
  /** Remote session-key custody, or null for frontend custody. */
  readonly sessionSigner: Readonly<PermissionSessionSigner> | null;
}

interface PermissionDecisionCommon {
  readonly version: typeof OAATH_PERMISSION_DECISION_VERSION;
  readonly requestId: string;
...
function invalid(code: PermissionProtocolErrorCode, message: string): never {
  throw new OaathPermissionProtocolError(code, message);
}
...
function captureFailure<Value>(code: PermissionProtocolErrorCode, action: () => Value): Value {
  try {
    ...
    if (error instanceof OaathPermissionProtocolError) throw error;
...
function captureSessionSigner(
...
): Readonly<PermissionSessionSigner> {
  const record = exactRecord(
    value,
    ["mode", "providerId"],
    "permission session signer",
    context,
    failFor(code),
  );
  ...
  if (
    ...
    record.providerId.length > MAX_REQUEST_ID_LENGTH
  ) {
    return invalid(code, "permission session signer provider is invalid");
  }
...
function captureRequest(
  ...
  if (record.version !== OAATH_PERMISSION_REQUEST_VERSION) {
...
function captureDecision(
  ...
  if (captured.kind === "reject") {
    ...
    if (record.version !== OAATH_PERMISSION_DECISION_VERSION) {
