...
const request: PermissionRequest = {
  version: "ogp.permission-request/v1",
...
function expectProtocolError(
  ...
  try {
    ...
    expect(error).toBeInstanceOf(OgpPermissionProtocolError);
...
describe("PermissionRequest current codec", () => {
  it("captures the exact application/account/operator/policy request and creates its Grant", () => {
    expect(OGP_PERMISSION_REQUEST_VERSION).toBe("ogp.permission-request/v1");
    expect(OGP_PERMISSION_REQUEST_HASH_DOMAIN).toBe("@leekt/ogp:permission-request");
  ...
  it("rejects wrong versions, chain modes, times, fields, profiles, and hostile records", () => {
    ...
    const invalidRequests: unknown[] = [
      { ...clone(request), version: "ogp.permission-request/v0" },
...
describe("PermissionDecision current codec", () => {
  it("owns closed approve/reject records and hashes every authority-bearing field", () => {
    expect(OGP_PERMISSION_DECISION_VERSION).toBe("ogp.permission-decision/v1");
    expect(OGP_PERMISSION_DECISION_HASH_DOMAIN).toBe("@leekt/ogp:permission-decision");
  ...
  it("rejects contradictory variants, hashes, numeric aliases, extras, and hostile fields", () => {
    ...
    const invalidDecisions: unknown[] = [
      { ...clone(approve()), version: "ogp.permission-decision/v0" },
