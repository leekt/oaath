...
function expectPreparedError(
  action: () => unknown,
  code: OgpPreparedUserOperationError["code"],
): void {
  try {
    action();
  } catch (error) {
    expect(error).toBeInstanceOf(OgpPreparedUserOperationError);
    expect((error as OgpPreparedUserOperationError).code).toBe(code);
    return;
  }
...
describe("prepared UserOperation identity", () => {
  ...
  it("rejects wrong record versions, unknown record fields, and malformed recorded hashes", () => {
    const prepared = prepareUserOperation(preparationInput());
    expectRecordInvalid({ ...prepared, version: "ogp.prepared-user-operation/v0" });
