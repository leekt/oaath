...
function expectPolicyError(action: () => unknown, code: OgpGrantPolicyError["code"]): void {
  try {
    action();
  } catch (error) {
    expect(error).toBeInstanceOf(OgpGrantPolicyError);
...
describe("canonical Grant policy", () => {
  it("round-trips one immutable current policy with a stable domain-separated encoding", () => {
    expect(OGP_GRANT_POLICY_VERSION).toBe("ogp.grant-policy/v1");
    expect(OGP_GRANT_POLICY_HASH_DOMAIN).toBe("@leekt/ogp:grant-policy");
    ...
    expect(hashGrantPolicy(parsed)).toBe(
      "0x582a24e8772042fad8d66f2db918d3743984ce4114c241b30ded4b56e711c3c5",
    );
  ...
  it("rejects aliases, malformed encodings, overflow, duplicates, and noncanonical order", () => {
    ...
    const invalidPolicies: unknown[] = [
      { ...clone(policy), version: "ogp.grant-policy/v0" },
