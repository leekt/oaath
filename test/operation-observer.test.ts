...
function expectObserverError(action: () => unknown, code: OgpOperationObserverError["code"]): void {
  try {
    action();
  } catch (error) {
    expect(error).toBeInstanceOf(OgpOperationObserverError);
    expect((error as OgpOperationObserverError).code).toBe(code);
    return;
  }
...
describe("OperationObserver", () => {
  ...
  it("continues finality from a durable Operation after recreating store and observer", async () => {
    const directory = await mkdtemp(join(tmpdir(), "ogp-observer-reload-"));
