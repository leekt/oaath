...
export const OGP_GRANT_POLICY_VERSION = "ogp.grant-policy/v1" as const;
export const OGP_GRANT_POLICY_USAGE_VERSION = "ogp.grant-policy-usage/v1" as const;
export const OGP_GRANT_POLICY_HASH_DOMAIN = "@leekt/ogp:grant-policy" as const;
...
export interface GrantPolicy {
  readonly version: typeof OGP_GRANT_POLICY_VERSION;
...
export interface CompleteGrantPolicyUsageEvidence {
  readonly version: typeof OGP_GRANT_POLICY_USAGE_VERSION;
...
function invalid(code: GrantPolicyErrorCode, message: string): never {
  throw new OgpGrantPolicyError(code, message);
}
...
function captureFailure<Value>(code: GrantPolicyErrorCode, action: () => Value): Value {
  try {
    ...
    if (error instanceof OgpGrantPolicyError) throw error;
...
function capturePolicy(
  ...
  if (record.version !== OGP_GRANT_POLICY_VERSION) {
...
function parseUsageEvidence(
  ...
  if (captured.status === "complete") {
    ...
    if (record.version !== OGP_GRANT_POLICY_USAGE_VERSION) {
      return invalid(code, "grant policy usage evidence version is unsupported");
    }
    return Object.freeze({
      version: OGP_GRANT_POLICY_USAGE_VERSION,
      status: "complete",
  ...
  if (record.version !== OGP_GRANT_POLICY_USAGE_VERSION || record.status !== "unavailable") {
