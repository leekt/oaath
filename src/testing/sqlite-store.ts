...
const SQLITE_SCHEMA_VERSION = "ogp.sqlite-test-store/v1";

const METADATA_SCHEMA = `
  CREATE TABLE ogp_test_store_schema_v1 (
    schema_id TEXT PRIMARY KEY CHECK (schema_id = 'ogp'),
    version TEXT NOT NULL
  ) STRICT, WITHOUT ROWID
`;

const GRANT_SCHEMA = `
  CREATE TABLE ogp_test_grant_store_v1 (
...
const OPERATION_SCHEMA = `
  CREATE TABLE ogp_test_operation_store_v1 (
...
const EXPECTED_SCHEMAS = new Map([
  ["ogp_test_store_schema_v1", METADATA_SCHEMA],
  ["ogp_test_grant_store_v1", GRANT_SCHEMA],
  ["ogp_test_operation_store_v1", OPERATION_SCHEMA],
]);
...
function validateSchema(database: DatabaseSync): void {
  ...
  if (objects.length !== EXPECTED_SCHEMAS.size) {
    throw new OgpStoreError("store_record_invalid", "SQLite test store schema is unsupported");
  }
  ...
  const versions = database
    .prepare("SELECT schema_id, version FROM ogp_test_store_schema_v1")
    .all() as Array<{ schema_id: unknown; version: unknown }>;
  if (
    versions.length !== 1 ||
    versions[0]?.schema_id !== "ogp" ||
    versions[0]?.version !== SQLITE_SCHEMA_VERSION
  ) {
    throw new OgpStoreError(
...
function initializeOrValidateSchema(database: DatabaseSync): void {
  ...
  try {
    ...
    if (objectCount?.count === 0 || objectCount?.count === 0n) {
      ...
      database
        .prepare("INSERT INTO ogp_test_store_schema_v1 (schema_id, version) VALUES (?, ?)")
        .run("ogp", SQLITE_SCHEMA_VERSION);
    }
...
function openDatabase(filePath: string): DatabaseSync {
  if (typeof filePath !== "string" || filePath.length === 0 || filePath === ":memory:") {
    throw new OgpStoreError("store_input_invalid", "SQLite test store requires a file path");
  }
  ...
  try {
    ...
    if (error instanceof OgpStoreError) throw error;
    throw new OgpStoreError("store_unavailable", "SQLite test store could not be opened");
  }
}
...
function prepareStore<Value>(database: DatabaseSync, prepare: () => Value): Value {
  try {
    ...
    throw new OgpStoreError("store_unavailable", "SQLite test store could not be prepared");
  }
}
...
export function createSqliteGrantStore(filePath: string): GrantStore {
  ...
  return prepareStore(database, () => {
    ...
    FROM ogp_test_grant_store_v1
    WHERE grant_id = ?
  `);
    const insert = database.prepare(`
    INSERT INTO ogp_test_grant_store_v1 (
      grant_id, record_version, store_revision, updated_at, payload
    ) VALUES (?, ?, ?, ?, ?)
    ON CONFLICT (grant_id) DO NOTHING
  `);
    const update = database.prepare(`
    UPDATE ogp_test_grant_store_v1
...
export function createSqliteOperationStore(filePath: string): OperationStore {
  ...
  return prepareStore(database, () => {
    const get = database.prepare(`
    SELECT record_version, store_revision, updated_at, payload
    FROM ogp_test_operation_store_v1
    WHERE grant_id = ? AND chain_id = ?
  `);
    const insert = database.prepare(`
    INSERT INTO ogp_test_operation_store_v1 (
